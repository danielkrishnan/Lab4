...
def data_taker():
    ...
    for i in range (0, len(l_long), 1):
        sys.stdout = open('obs_log_Apr_30.txt', 'a') 
        #update pointing                
        ...
        try:         
            ...
            try:
                s.set_amp(10.0)
                #lower LO freq, noise on 
                d.noise_on()
                LST = obs.sidereal_time()
                takespec.takeSpec('spec_low_freq_noise_on_%s_%s_%s.gz' % (b, l, LST), numSpec=80)
                print 'low freq noise on spectra done'
                #lower LO freq, noise off 
                d.noise_off() 
                LST = obs.sidereal_time()
                takespec.takeSpec('spec_low_freq_noise_off_%s_%s_%s.gz' % (b, l, LST), numSpec=180)
                print 'low freq noise off spectra done' 
            except ValueError:
        ...
        try:
            ...
            try:
                ...
                d.noise_on()
                LST = obs.sidereal_time()
                takespec.takeSpec('spec_up_freq_noise_on_%s_%s_%s.gz' % (b, l, LST), numSpec=80)
                print 'upper lo freq noise on spectra done' 
                d.noise_off()
                LST = obs.sidereal_time()
                takespec.takeSpec('spec_up_freq_noise_off_%s_%s_%s.gz' % (b, l, LST), numSpec=180)
                print 'upper lo freq noise off spectra done'
            except ValueError:
                print "Could not take spectra, skipping point"
                pass
        except ValueError:
